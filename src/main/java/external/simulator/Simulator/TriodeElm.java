...
public class TriodeElm extends CircuitElm {
	...
	public void draw(CircuitGUI.ClientCircuitGui g, int mouseX, int mouseY, float partialTicks) {
		drawThickCircle(g, point2.getX(), point2.getY(), circler, Color.lightGray);
		...
		int i;
		for (i = 0; i != 8; i += 2)
			drawThickLine(g, grid[i], grid[i + 1], getVoltageColor(volts[1]));
		// draw cathode
		for (i = 0; i != 3; i++)
			drawThickLine(g, cath[i], cath[i + 1], getVoltageColor(volts[2]));
		// draw dots
