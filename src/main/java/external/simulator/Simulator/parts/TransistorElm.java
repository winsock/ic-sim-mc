...
import external.simulator.Simulator.Scope;
...
public class TransistorElm extends CircuitElm {
	...
	public void draw(CircuitGUI.ClientCircuitGui g, int mouseX, int mouseY, float partialTicks) {
		...
		// draw collector
		drawThickLine(g, coll[0], coll[1], getVoltageColor(volts[1]));
		// draw emitter
		drawThickLine(g, emit[0], emit[1], getVoltageColor(volts[2]));
		// draw arrow
		drawPolygon(g, arrowPoly, lightGrayColor);
		// draw base
		drawThickLine(g, point1, base, getVoltageColor(volts[0]));
		// draw dots
		...
		// draw base rectangle
		drawPolygon(g, rectPoly, getVoltageColor(volts[0]));

		if ((needsHighlight() || sim.getDragElm() == this) && dy == 0) {
			int ds = sign(dx);
			FontRenderer renderer = Minecraft.getMinecraft().fontRendererObj;
			g.drawString(renderer, "B", base.getX() - 10 * ds, base.getY() - 5, Color.WHITE.getRGB());
			g.drawString(renderer, "C", coll[0].getX() - 3 + 9 * ds, coll[0].getY() + 4, Color.WHITE.getRGB()); // x+6 if ds=1, -12 if -1
			g.drawString(renderer, "E", emit[0].getX() - 3 + 9 * ds, emit[0].getY() + 4, Color.WHITE.getRGB());
		}
	...
	public EditInfo getEditInfo(int n) {
		...
		if (n == 1) {
			...
			ei.setCheckbox(new Toggleable("Swap E/C", (flags & FLAG_FLIP) != 0));
			return ei;
		}
		return null;
	}
