...
/*    void setSize(int s) {
    ...
    void drawChip(GuiScreen g) {
        ...
        for (i = 0; i != getPostCount(); i++) {
            Pin p = pins[i];
            Color voltageColor = (Color) getVoltageColor(volts[i]);
            Point a = p.post;
            Point b = p.stub;
            ...
            if (p.bubble) {
                drawThickCircle(g, p.bubbleX, p.bubbleY, 1, (Color) Color.WHITE);
                drawThickCircle(g, p.bubbleX, p.bubbleY, 3, (Color) Color.LTGREY);
            }
            FontRenderer renderer = Minecraft.getMinecraft().fontRendererObj;
            int sw = renderer.getStringWidth(p.text);
            g.drawString(renderer, p.text, p.textloc.getX() - sw / 2, p.textloc.getY() + renderer.FONT_HEIGHT / 2, Color.WHITE.hashCode());
            if (p.lineOver) {
                int ya = p.textloc.getX() - renderer.FONT_HEIGHT / 2;
                Tessellator tessellator = Tessellator.getInstance();
                WorldRenderer worldRenderer = tessellator.getWorldRenderer();
                worldRenderer.startDrawing(GL11.GL_LINE);
                worldRenderer.setColorRGBA(Color.WHITE.getRed(), Color.WHITE.getGreen(), Color.WHITE.getBlue(), Color.WHITE.getAlpha());
                worldRenderer.addVertex(p.textloc.getX() - sw / 2, ya, 0);
                worldRenderer.addVertex(p.textloc.getX() + sw / 2, ya, 0);
        ...
        if (clockPointsX != null) {
            ...
            for (int x = 0; x < 3; x++) {
                worldRenderer.addVertex(clockPointsX[x], clockPointsY[x], 0);
            }
            worldRenderer.finishDrawing();
        }
        for (i = 0; i != getPostCount(); i++)
